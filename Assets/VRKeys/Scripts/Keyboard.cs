 ...
 * * MODIFIED FOR UNITY 6 & NEW INPUT SYSTEM COMPATIBILITY
 */

using UnityEngine;
// using UnityEngine.XR; // Disable Legacy XR
...
namespace VRKeys {
	...
	public class Keyboard : MonoBehaviour {
		...
		// [FIX UNITY 6] Removed legacy private variables
		...
		private IEnumerator Start () {
			// [FIX UNITY 6] Removed Legacy VR Setup that causes errors
			...
			if(validationNotice) validationNotice.SetActive (false);
			if(infoNotice) infoNotice.SetActive (false);
			if(successNotice) successNotice.SetActive (false);
		...
		private void Update () {
			// [FIX UNITY 6] Disable Update Loop
			// We do NOT want to update hand positions using legacy InputTracking.
			// Your XR Rig handles the hands now.
		}

		private void PositionAndAttachMallets () {
			// [FIX UNITY 6] Removed Auto-Parenting logic.
			// You must parent Mallets manually in the Hierarchy to your Controllers.
			...
			if (leftMallet != null) leftMallet.SetActive (true);
			...
			// Ensure they are active
			if (rightMallet != null) rightMallet.SetActive (true);
		}
